...
export default function PTReport({ 
  ...
  useEffect(() => {
    if (open) {
        const isEditing = !!poiToEdit && poiToEdit.type === 'pt';
    ...
    <Sheet open={open} onOpenChange={(isOpen) => {
      ...
      >
        <SheetHeader className="p-6 pb-2">
          <SheetTitle>{isEditMode ? 'Editar Posto de Transformação' : 'Mapear Posto de Transformação (PT)'}</SheetTitle>
          <SheetDescription>
             Ajuste o pino no mapa para a localização exata do PT e preencha os seus dados técnicos.
          </SheetDescription>
        </SheetHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
                 <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                        <FormLabel>ID / Código do PT</FormLabel>
                        <FormControl>
                            <Input placeholder="Ex: PT-01-A2" {...field} />
                        </FormControl>
                        <FormMessage />
                        </FormItem>
                    )}
            ...
            <SheetFooter className="p-6 pt-4 border-t bg-background">
                ...
                <Button type="submit">{isEditMode ? 'Guardar Alterações' : 'Registar PT'}</Button>
            </SheetFooter>
          </form>
        </Form>
      </SheetContent>
